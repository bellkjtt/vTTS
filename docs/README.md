# vTTS Documentation

[🏠 홈으로 돌아가기](../README.md)

vTTS 프로젝트의 추가 문서 모음입니다.

## 📚 문서 목록

### 개발자 가이드
- **[PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md)**: 프로젝트 구조 및 아키텍처
- **[PROJECT_STATUS.md](PROJECT_STATUS.md)**: 프로젝트 현황 및 로드맵

### 배포 및 릴리스
- **[DEPLOYMENT_SUCCESS.md](DEPLOYMENT_SUCCESS.md)**: 배포 성공 체크리스트
- **[RELEASE_CHECKLIST.md](RELEASE_CHECKLIST.md)**: 릴리스 체크리스트

### 커뮤니티 및 관리
- **[GITHUB_SETUP.md](GITHUB_SETUP.md)**: GitHub 저장소 설정 가이드
- **[SPONSORS_SETUP.md](SPONSORS_SETUP.md)**: 스폰서 설정 가이드

## 🔗 주요 문서 (루트)

프로젝트 루트에 있는 주요 문서:

- **[README.md](../README.md)**: 프로젝트 메인 문서 (한국어)
- **[README_EN.md](../README_EN.md)**: English documentation
- **[README_ZH.md](../README_ZH.md)**: 中文文档
- **[README_JA.md](../README_JA.md)**: 日本語ドキュメント
- **[INSTALL.md](../INSTALL.md)**: 설치 가이드
- **[QUICKSTART.md](../QUICKSTART.md)**: 빠른 시작 가이드
- **[DOCKER.md](../DOCKER.md)**: Docker 사용 가이드
- **[TROUBLESHOOTING.md](../TROUBLESHOOTING.md)**: 문제 해결 가이드
- **[ENGINES_SETUP.md](../ENGINES_SETUP.md)**: 엔진별 설정 가이드

## 🧪 테스트 문서

- **[tests/README.md](../tests/README.md)**: 테스트 가이드 및 Kaggle 노트북

## 📦 Examples

- **[examples/](../examples/)**: 사용 예제 코드

## 🤝 기여

문서 개선을 원하시면 PR을 보내주세요!
